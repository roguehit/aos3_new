...
void* sync_func( void* t ) 
{
    ...
    xmlrpc_client_create(arg->envP, XMLRPC_CLIENT_NO_FLAGS, NAME, VERSION, NULL, 0, &arg->client);
...
#if 1 
    validateGlobalClientExists(arg->envP);
    if (!arg->envP->fault_occurred) {
        xmlrpc_value * paramArrayP;
        const char * suffix;

        xmlrpc_build_value_va(arg->envP, arg->format, arg->args, &paramArrayP, &suffix);

        if (!arg->envP->fault_occurred) {
            if (*suffix != '\0')
                xmlrpc_faultf(arg->envP, "Junk after the argument "
                              "specifier: '%s'.  "
                              "There must be exactly one arument.",
                              suffix);
            else
                xmlrpc_client_call2(arg->envP, arg->client, serverInfoP,
                                    arg->methodName, paramArrayP, &resultP);
